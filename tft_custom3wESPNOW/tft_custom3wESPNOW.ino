...
#include "functioners.h"
#include "array"



using namespace std;
//Sketch uses 954526 bytes (72%) of program storage space. Maximum is 1310720 bytes.
//Global variables use 44676 bytes (13%) of dynamic memory, leaving 283004 bytes for local variables. Maximum is 327680 bytes.
array<int16_t, 8> classButtonData;
array<ValueChangeFunctioner, 8> classfunctionerData;

ValueChangeFunctioner buttonFunctions[8] = {};
int16_t buttonData[8] = {};
int16_t unusedData;
...
//Honestly I think we can get rid of touch info
class TouchInfo {
public:
  uint16_t t_x;
  uint16_t t_y;
...
//array<DButton,NUMOFKEYS> key{{DButton("Red"),DButton("Blue"),DButton("Green")}};
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
  ...
  incomingGRN = incomingReadings.GRN;
  incomingBLU = incomingReadings.BLU;
  Serial.println(incomingRED);
  Serial.println(incomingGRN);
  Serial.println(incomingBLU);
}
...
void setup() {
  ...
  for (int i = 0; i < 8; i++) {
    buttonFunctions[i].assignValueChange(unusedData, 1, 0, 100);
  }
  Serial.print("height   ");
  Serial.println(gTft.height());
  Serial.print("width   ");
  Serial.println(gTft.width());
...
void loop(void) {
  ...
  runScreen(gTft);
  //runScreenSimple(gTft, buttonFunctions,buttonData);
...
void drawKeypad()  // args  tft, keys
{
  ...
  def.createButton(key[2], styler, 1, 0, numRow, 3);  // this and the line above row draw the same button.

  key[2].drawButton();
}
//change to
//template < size_t, N>
//array<TFT_eSPI_Button,N> &key,
void hasTouchScreenBeenTouched(TFT_eSPI_Button *key, TouchInfo touchInfo) {
  ...
  for (uint8_t b = 0; b < touchInfo.numOfKeys; b++) {
    if (touchInfo.pressed && key[b].contains(touchInfo.t_x, touchInfo.t_y)) {  //you need touchinfo.pressed && because you may have data from previous press still
...
void runScreenSimple(TFT_eSPI &tft, ValueChangeFunctioner *buttonFuncs, int16_t *buttonDatas) {
}
//runSceenStill has touchInfo as global data
void runScreen(TFT_eSPI &tft) {  //args key, data // mutex?  chage gTft to tft

  tftElementDefiner def(tft, tft.width(), tft.height());  //def.setScreen(gTft, 240,320);
  tftStyler textBoxStyler;

  //used for text box
  textBoxStyler.colors(tft.color565(0, 32, 32), tft.color565(0, 255, 255), pink);
  ...
  touchInfo.pressed = tft.getTouch(&touchInfo.t_x, &touchInfo.t_y);  //intersting you can def(*x){}; def(&x); to get the same as def(&x){} def(x);
